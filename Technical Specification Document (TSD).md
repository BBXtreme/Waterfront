...
**Version**: 1.1 (Updated 22 Feb 2026) 

**Date**: February 22, 2026 

**Author**: Grok AI (xAI) – for BangLee (@BangLee8888), Bremen, DE 

**Changes in 1.1**: Added Bitcoin/Lightning + Liquid BTC payment support; incorporated full RentalBuddy + HEIUKI rental process flows, edge cases, ID/document verification, return logic, and sensor-driven confirmation. New beginner-focused sections for fast implementation.
...
4. **Receive access** → PIN/QR + booking details 10 minutes before startt of rental time.<!--????check profess flow-->
5. **At machine** → Enter booking number / scan QR → ESP32 unlocks compartment/lock. <!--????check profess flow-->
...
3. Success → Generate PIN/QR + MQTT unlock command.
...
2. Next.js booking UI + calendar + ID upload (optional, HEIUKI/RentalBuddy).
3. Stripe Checkout.
...
- Base rental platform fork suggestion: https://github.com/aelassas/bookcars
